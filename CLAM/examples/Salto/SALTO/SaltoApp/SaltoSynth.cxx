...
bool SaltoSynth::ConcreteConfigure( const ProcessingConfig& cfg)throw( std::bad_cast )
{
	...
	mSineSynthPO.Configure( sineCfg );

	InterpolatingSynthesisConfig interpCfg;
	interpCfg.SetSpectralRange( mConfig.GetSpectralRange() );

	mInterpolPO.Configure( interpCfg );
	
	mSampleStepping = mConfig.GetAnalysisFrameSize();

	LinkOutWithInControl( 0, &mInterpolPO, 0 );
	LinkOutWithInControl( 1, &mInterpolPO, 1 );
	LinkOutWithInControl( 2, &mInterpolPO, 2 );
	LinkOutWithInControl( 3, &mInterpolPO, 3 );
	LinkOutWithInControl( 4, &mInterpolPO, 4 );
	LinkOutWithInControl( 5, &mInterpolPO, 5 );
	LinkOutWithInControl( 6, &mInterpolPO, 6 );
	LinkOutWithInControl( 7, &mInterpolPO, 7 );
	LinkOutWithInControl( 8, &mInterpolPO, 8 );
	LinkOutWithInControl( 9, &mInterpolPO, 9 );

	LinkOutWithInControl( 10, &mSineSynthPO, 0 );
	LinkOutWithInControl( 11, &mSineSynthPO, 1 );
	LinkOutWithInControl( 12, &mSineSynthPO, 2 );
	LinkOutWithInControl( 13, &mSineSynthPO, 3 );

	LinkOutWithInControl( 14, &mAttackResSynthPO,   0 );
	LinkOutWithInControl( 14, &mStatResSyhthesisPO, 0 );
	LinkOutWithInControl( 15, &mStatResSyhthesisPO, 1 );

	mSineSynthPO.LinkOutWithInControl( 0, this, 1 );
	mSineSynthPO.LinkOutWithInControl( 1, this, 2 );
...
bool SaltoSynth::ConcreteStart()
{
	...
	mInterpolPO.Start();
	mSineSynthPO.Start();
	mAttackResSynthPO.Start();
	mStatResSyhthesisPO.Start();
...
bool SaltoSynth::ConcreteStop()
{
	...
	mInterpolPO.Stop();
	mSineSynthPO.Stop();
	mAttackResSynthPO.Stop();
	mStatResSyhthesisPO.Stop();
...
void SaltoSynth::HandleAttack(Audio*& synthBuffer)
{
	...
	mSineSynthPO.ResetSineSynthesis();
...
void SaltoSynth::DoInterpolatingReleaseSynthesis( CSaltoSynthFrame* pSynthFrame, double gain, int currRelFrame )
{
	...
	mInterpolPO.DoMagInterpolation( *mpCurrSpectralFrameBase,
...
void SaltoSynth::DoTransitionSynthesis( CSaltoSynthFrame* pSynthFrame )
{
	...
	*/
	...
	mInterpolPO.DoInterpolation(* mpTransitionFrameBase,
...
void SaltoSynth::AttackResidualSynthesis( CSaltoSynthFrame* pSynthFrame )
{
	...
	if ( !ipFactor )
	{
		...
		mAttackResSynthPO.Do( *pSynthFrame );
	}
	else
		{
	...
	if ( ipFactor > 1 )
		{
			...
			mStatResSyhthesisPO.Do( *pSynthFrame, *pStatResidual );
		}
	else
		{
