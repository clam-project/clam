	...
	namespace OnsetDetection
	{
		...
		bool AudioDecimator::Do(DataArray &input, int sampleRate, int type)
		{
			...
			case 3:
				...
				//coefs from Matlab for an elliptic filter fs=22.05k, FPass=2000, FStop=2200.
				if ( mOutput.Size() < input.Size() )
				{
					mOutput.Resize( input.Size() );
				}								

				mOutput.SetSize( input.Size() );
				...
				for(i=0; i<size; i++)
				{
					if(i==0) mOutput[i] = a0*input[i];
					else if(i==1) mOutput[i] = a0*input[i]+ a1*input[i-1]- b1*mOutput[i-1];
					else if(i==2) mOutput[i] = a0*input[i]+ a1*input[i-1] + a2*input[i-2] - b1*mOutput[i-1] - b2*mOutput[i-2];
					else if(i==3) mOutput[i] = a0*input[i]+ a1*input[i-1] + a2*input[i-2] + a3*input[i-3] - b1*mOutput[i-1] - b2*mOutput[i-2] - b3*mOutput[i-3];
					else if(i==4) mOutput[i] = a0*input[i]+ a1*input[i-1] + a2*input[i-2] + a3*input[i-3]  + a4*input[i-4]
							      - b1*mOutput[i-1] - b2*mOutput[i-2] - b3*mOutput[i-3] - b4*mOutput[i-4];
					else if(i==5) mOutput[i] = a0*input[i]+ a1*input[i-1] + a2*input[i-2] + a3*input[i-3]  + a4*input[i-4]+ a5*input[i-5]
							      - b1*mOutput[i-1] - b2*mOutput[i-2] - b3*mOutput[i-3] - b4*mOutput[i-4]- b5*mOutput[i-5];
					else if(i==6) mOutput[i] = a0*input[i]+ a1*input[i-1] + a2*input[i-2] + a3*input[i-3]  + a4*input[i-4]+ a5*input[i-5]+ a6*input[i-6]
							      - b1*mOutput[i-1] - b2*mOutput[i-2] - b3*mOutput[i-3] - b4*mOutput[i-4]- b5*mOutput[i-5]- b6*mOutput[i-6];
					else if(i==7) mOutput[i] = a0*input[i]+ a1*input[i-1] + a2*input[i-2] + a3*input[i-3]  + a4*input[i-4]+ a5*input[i-5]+ a6*input[i-6]+ a7*input[i-7]
							      - b1*mOutput[i-1] - b2*mOutput[i-2] - b3*mOutput[i-3] - b4*mOutput[i-4]- b5*mOutput[i-5]- b6*mOutput[i-6]- b7*mOutput[i-7];
					else mOutput[i] = a0*input[i] + a1*input[i-1] + a2*input[i-2] + a3*input[i-3] + a4*input[i-4] + a5*input[i-5]
						     + a6*input[i-6] + a7*input[i-7] + a8*input[i-8]	- b1*mOutput[i-1] - b2*mOutput[i-2] - b3*mOutput[i-3] - b4*mOutput[i-4] - b5*mOutput[i-5] - b6*mOutput[i-6] - b7*mOutput[i-7] - b8*mOutput[i-8];
				}
			
/*			input.Resize(size/5);
			input.SetSize(size/5);
