...
namespace CLAM {
	...
	void TextFileMIDIDevice::ConcreteStart(void)
	{
		if (mOut) fclose(mOut);
		if (mIn) fclose(mIn);
	...
	void TextFileMIDIDevice::ConcreteStop(void)
	{
		if (mOut) fclose(mOut);
		if (mIn) fclose(mOut);
		mOut = 0;
	...
	void TextFileMIDIDevice::Write(unsigned char* msg,int size)
	{
	...
	void TextFileMIDIDevice::Read(void)
	{
	}
