...
#include "Fl_Browsable_Audio.hxx"
...
namespace CLAMVM
{
	Fl_Browsable_Audio::Fl_Browsable_Audio( int X, int Y, int W, int H, const char* label )
		: Fl_Window( X, Y, W, H, label )
	{
		...
		mDisplay = new Fl_Gl_Single_Browsable_Display( 0,0,0,0 );
	...
	void Fl_Browsable_Audio::resize( int x, int y, int w, int h )
	{
	...
	Fl_Browsable_Audio::~Fl_Browsable_Audio( )
	{
	}

	void Fl_Browsable_Audio::OnNewAudio( const DataArray& array, TTime begin, TTime end, TData srate )
	{
	...
	void Fl_Browsable_Audio::Show()
	{
		show();
	}

	void Fl_Browsable_Audio::Hide()
	{
		hide();
	}

	void Fl_Browsable_Audio::SetPainting(bool painting) 
	{ 
		mDisplay->SetPainting(painting); 
	}
		
	void Fl_Browsable_Audio::SetPos( CLAM::TData pos )
	{ 
		mDisplay->SetPos(pos); 
	}
		
	Slotv1<TData>* Fl_Browsable_Audio::GetFrameSlot()
	{
		return mDisplay->GetFrameSlot();

	}

	Slotv1<bool>* Fl_Browsable_Audio::GetPaintSlot() 
	{ 
		return mDisplay->GetPaintSlot(); 
	} 

	Signalv1<double>* Fl_Browsable_Audio::GetSignal() 
	{ 
