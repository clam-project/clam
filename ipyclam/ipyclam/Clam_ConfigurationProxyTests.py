from .clam import configurationproxy
...
class Clam_ConfigurationProxyTests(unittest.TestCase):
	...
	def test_createConfigurationProxy(self):
		engine = configurationproxy.createConfigurationProxy("DummyProcessingWithStringConfiguration")
		self.assertEqual(
	...
	def test_getDefaultValue_whenInt(self):
		engine = configurationproxy.createConfigurationProxy("DummyProcessingWithIntConfiguration")
		self.assertEqual(42, engine["FirstInt"])
		self.assertEqual(15, engine["SecondInt"])

	def test_getDefaultValue_whenOtherTypes(self):
		engine = configurationproxy.createConfigurationProxy("DummyProcessingWithCompleteConfiguration")
	...
	def test_setAttr_wrongName(self):
		c = configurationproxy.createConfigurationProxy("DummyProcessingWithIntConfiguration")
		try:
			c["WrongParam"] = 24
			self.fail("Exception expected")
		except KeyError as e:
	...
	def test_set_wrongTypeForString(self):
		c = configurationproxy.createConfigurationProxy("DummyProcessingWithStringConfiguration")
		try:
			c["AString"] = 2
			self.fail("Exception expected")
	...
	def test_nonDefault(self):
		engine = configurationproxy.createConfigurationProxyWithProc("AudioSource")
	...
	def test_hold_apply(self):
		c = configurationproxy.createConfigurationProxyWithProc("AudioSource")
		c.hold()
		c["NSources"] = 2
	...
	def test_copyConfig(self):
		engine = configurationproxy.createConfigurationProxyWithProc("AudioSource")
		config = configurationproxy.createConfigurationProxyWithProc("AudioSource")
		config["NSources"] = 2
		self.assertEqual(1, engine["NSources"])
	...
	def test_get_optional_attribute(self):
		engine = configurationproxy.createConfigurationProxy("DummyProcessingWithOptionalAttributes")
